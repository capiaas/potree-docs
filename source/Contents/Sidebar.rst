...
Point Budget Slider
------------------------
This is the same function as the point budget slider in the Web interface. 
...
To control viewable range that what is included within the scene, the field of view can be adjusted. Default value is set to 55 degrees.
...
measurement tools also can be found in the web interface. To remove all measurements that have been made, click on the red cross or press "Escape". 
...
This is the same function as the angle measurement tool in the Web interface. 
...
This is the same function as the point measurement tool in the Web interface. 
...
This is the same function as the distance measurement tool in the Web interface. 
...
This is the same function as the height measurement tool in the Web interface. 
...
This is the same function as the circle measurement tool in the Web interface. 
...
This tool measures the azimuthal angle of a line. This line is formed by two points selected by the user, the angle is measured in degrees, 
clockwise from 0 to 360 and starting from the geographical north. To start a measurement, click on the azimuth icon and then click the first position for the center point,
then click the second position to complete the measurement.
...
This tool measures the volume formed by a sphere. To start a measurement, click on the volume (sphere) icon and click on the model to place the sphere. It is possible to relocate, 
rescale and rotate the sphere using the displayed handlers. Right click to finish measurement. 
Further information can also be obtained from selecting this element under the scene section.
...
This tool creates a height profile formed by a line on the model. To start a measurement, click on the Height profile icon and then form a line on the pointcloud by 
clicking on the desired points (two or more). 
...
When you want to finalize your profile double click on the last node. Once a profile is finalized, you can still change its location by clicking on the red nodes and dragging 
...
To edit the annotation, select this element under the scene section, then edit Title and Description. Click the annotation can zoom to the annotation.
...
This is the same navigation mode as the earthcontrol in the Web interface. 
...
This is the same navigation mode as the flycontrol in the Web interface. 
